...
function renderKeywordTable(data = appState.keywordsData) {
        ...
        div.innerHTML = `
            <div class="result-header">
                ...
                <div class="result-actions">
                    ...
                    <button class="btn-icon-text" onclick="exportToDocx(${index})" title="Download Word (.docx)">
                        <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                        Word
                    </button>
                     <button class="btn-icon-text" onclick="exportToPdf(${index})" title="Download PDF">
                        <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                        PDF
                    </button>
                   <button class="btn-icon-text" onclick="viewBlogPreview(${index})" title="View Blog Format">
...
function prepareExportContent(item) {
    let content = item.content || '';
    ...
    const desc = item.intent || '';

    return `
        <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; line-height: 1.6; color: #333;">
            <h1 style="color: #202124; font-size: 24px; border-bottom: 2px solid #eee; padding-bottom: 10px;">${title}</h1>
            ${desc ? `<p style="font-style: italic; color: #5f6368; background: #f8f9fa; padding: 10px; border-left: 4px solid #1a73e8;"><strong>Meta Description:</strong> ${desc}</p>` : ''}
            <div style="margin-top: 20px;">
                ${content}
            </div>
             <div style="margin-top: 40px; border-top: 1px solid #eee; padding-top: 20px; font-size: 12px; color: #999; text-align: center;">
                Generated by SEO Content Planner
            </div>
        </div>
    `;
}

function exportToDocx(index) {
    const item = appState.processedResults[index];
    if (!item) return;

    const htmlContent = prepareExportContent(item);

    // Add Microsoft Word specific XML/HTML wrapper
    ...
    const filename = `${item.url || 'bai-viet'}.doc`;
...
function exportToPdf(index) {
    ...
    element.innerHTML = prepareExportContent(item);

    // Temporary append to body to render (hidden)
    ...
    const checkImages = element.querySelectorAll('img');
    const promises = [];
    ...
    Promise.all(promises).then(() => {
        const opt = {
            ...
            filename: `${item.url || 'bai-viet'}.pdf`,
